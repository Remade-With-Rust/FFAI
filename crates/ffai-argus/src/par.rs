...
//!
//! Argus reaches rayon throughout the vision tower — `siglip` fans out over
//! rows, hidden units and attention chunks at a dozen sites, `preprocess` over
//! resampled image rows, `text` over the decoder's key/value chunks — so an
//! unshimmed browser build crashes inside the first forward pass rather than
//! at load.
//!
