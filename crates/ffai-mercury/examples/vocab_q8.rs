...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let dev=Device::Cpu; let (d,vocab)=(384usize,51864usize);
    ...
    let f16v: Vec<f32> = Tensor::cat(&vec![&x16;pad],0)?.matmul(&wt16)?.narrow(0,0,1)?.to_dtype(DType::F32)?.flatten_all()?.to_vec1()?;
    let q8v:  Vec<f32> = q8.forward(&x)?.flatten_all()?.to_vec1()?;
    let e=|v:&Vec<f32>| v.iter().zip(&refv).map(|(a,b)|(a-b).abs()).fold(0f32,f32::max);
    // does the ARGMAX agree? that is what actually decides a token
    let am=|v:&Vec<f32>| v.iter().enumerate().max_by(|a,b|a.1.total_cmp(b.1)).unwrap().0;
    println!("accuracy vs f32: f16 max|d| {:.3e} argmax {}   q8_0 max|d| {:.3e} argmax {}   ref argmax {}",
        e(&f16v), am(&f16v), e(&q8v), am(&q8v), am(&refv));
    let rounds=41; let (mut w,mut va,mut vb)=(0usize,vec![],vec![]);
    let a=|| t(|| { std::hint::black_box(q8.forward(&x).unwrap()); });
    let b=|| t(|| { std::hint::black_box(Tensor::cat(&vec![&x16;pad],0).unwrap().matmul(&wt16).unwrap().narrow(0,0,1).unwrap()); });
    a(); b();
    for i in 0..rounds {
        let (p,q)= if i%2==0 { let p=a(); (p,b()) } else { let q=b(); (a(),q) };
    ...
    println!("  Q8_0 won {w}/{rounds} (z={z:+.1}) ratio {:.2}x -> {}", vb[rounds/2]/va[rounds/2],
        if z.abs()<2.0 {"INCONCLUSIVE"} else if z>0.0 {"Q8_0 FASTER"} else {"f16 FASTER"});
    Ok(())
}
