...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let dev=Device::Cpu; let (h,seq,hd)=(6usize,1500usize,64usize);
    ...
    let f16chain = |_:()| { let qk=q16.matmul(&k16).unwrap(); let w=fast_softmax(&qk).unwrap(); std::hint::black_box(w.matmul(&v16).unwrap()); };
    let f32chain = |_:()| { let qk=q32.matmul(&k32).unwrap(); let w=fast_softmax(&qk).unwrap(); std::hint::black_box(w.matmul(&v32).unwrap()); };
    let rounds=15; let (mut w,mut va,mut vb)=(0usize,vec![],vec![]);
    t(||f16chain(())); t(||f32chain(()));
    for i in 0..rounds {
        let (p,q) = if i%2==0 { let p=t(||f16chain(())); (p,t(||f32chain(()))) }
                    else { let q=t(||f32chain(())); (t(||f16chain(())),q) };
    ...
    println!("  paired f16 won {w}/{rounds} (z={z:+.1}) ratio {:.2}x -> {}",
        vb[rounds/2]/va[rounds/2],
        if z.abs()<2.0 {"INCONCLUSIVE"} else if z>0.0 {"f16 FASTER"} else {"f32 FASTER"});
    Ok(())
}
