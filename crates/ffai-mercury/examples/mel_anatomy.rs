...
use ffai_mercury::asr::mel::{HOP_LENGTH, MelSpectrogram, N_FFT, N_SAMPLES};
...
fn main() {
    ...
    println!(
        "WHOLE mel (30 s window): {:.2} ms   [whisper.cpp: ~4.4 ms/clip]",
        whole * 1e3
    );
    ...
    println!(
        "  1. {n_frames} FFTs of size {N_FFT}      {:7.2} ms  ({:.0}%)",
        t_fft * 1e3,
        t_fft / whole * 100.0
    );
    ...
    println!(
        "  2. filterbank 80x{n_bins} @ {n_bins}x{n_frames}  {:7.2} ms  ({:.0}%)  {:.1} GFLOP/s",
        t_fb * 1e3,
        t_fb / whole * 100.0,
        2.0 * (80 * n_bins * n_frames) as f64 / t_fb / 1e9
    );
