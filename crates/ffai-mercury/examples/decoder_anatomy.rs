...
fn best<T>(n: usize, mut f: impl FnMut() -> T) -> f64 {
    ...
    let mut b=f64::MAX; for _ in 0..n { let t=Instant::now(); let o=f(); std::hint::black_box(&o); b=b.min(t.elapsed().as_secs_f64()); } b
}
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let dev = Device::Cpu; let d = 384usize;
    println!("{:<28} {:>10} {:>12} {:>10} {:>12}", "OP (one decoder token)", "us", "weight KB", "GB/s", "% of 24GB/s");
    let mut rows: Vec<(String,f64,f64)> = Vec::new();
    let mut probe = |name: &str, k: usize, n: usize, dt: DType, count: usize| {
        let a = Tensor::zeros((1,k), DType::F32, &dev).unwrap().to_dtype(dt).unwrap();
        let b = Tensor::zeros((k,n), DType::F32, &dev).unwrap().to_dtype(dt).unwrap();
        let s = best(50, || a.matmul(&b).unwrap());
        let kb = (k*n) as f64 * if dt==DType::F32 {4.0} else {2.0} / 1e3;
        let gbs = kb*1e3/s/1e9;
        println!("{:<28} {:>10.1} {:>12.1} {:>10.1} {:>11.0}%", name, s*1e6, kb, gbs, gbs/24.0*100.0);
    ...
    let ln_w = Tensor::zeros(d, DType::F32, &dev)?; let ln_b = Tensor::zeros(d, DType::F32, &dev)?;
    let ln = candle_nn::LayerNorm::new(ln_w, ln_b, 1e-5);
    let lns = best(200, || <candle_nn::LayerNorm as candle_nn::Module>::forward(&ln, &t).unwrap());
    println!("\nper-op floor on a 1x384 tensor:  add {:.1} us · layer_norm {:.1} us", add*1e6, lns*1e6);
    ...
    println!("\nSUM of matmuls per token: {:.0} us, streaming {:.1} MB -> {:.1} GB/s effective",
        matmul_us, bytes_kb/1e3, bytes_kb*1e3/(matmul_us*1e-6)/1e9);
    println!("measured in-context: 6810 us/token  ->  overhead outside matmuls = {:.0} us ({:.0}%)",
        6810.0-matmul_us, (6810.0-matmul_us)/6810.0*100.0);
    Ok(())
}
