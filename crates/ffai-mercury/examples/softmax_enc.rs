...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let b: Vec<f32> = candle_nn::ops::softmax_last_dim(&x)?
        .flatten_all()?
        .to_vec1()?;
    println!(
        "correctness: max |delta| {:.2e}",
        a.iter()
            .zip(&b)
            .map(|(p, q)| (p - q).abs())
            .fold(0f32, f32::max)
    ...
    t(|| {
        std::hint::black_box(fast_softmax(&x).unwrap());
    });
    t(|| {
        std::hint::black_box(candle_nn::ops::softmax_last_dim(&x).unwrap());
    });
    for i in 0..rounds {
        let (p, q) = if i % 2 == 0 {
            let p = t(|| {
