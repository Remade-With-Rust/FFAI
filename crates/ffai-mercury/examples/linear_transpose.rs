...
fn probe(name: &str, k: usize, n: usize, dev: &Device) {
    ...
    let rounds = 201;
    let (mut win, mut va, mut vb) = (0usize, vec![], vec![]);
    let ours = || {
        t(|| {
            std::hint::black_box(
                x3.reshape((1, k))
    ...
    let theirs = || {
        t(|| {
            std::hint::black_box(lin.forward(&x3).unwrap());
        })
    };
    ours();
    theirs();
    ...
    println!(
        "  {name:<24} pre-transposed {:7.4} ms  Linear::forward {:7.4} ms  {win}/{rounds} z={z:+.1} ratio {:.2}x{}",
        va[rounds / 2],
        vb[rounds / 2],
