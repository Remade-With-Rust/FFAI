...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cont = t(9, || {
        std::hint::black_box(heads.contiguous().unwrap());
    });
    let kt = t(9, || {
        std::hint::black_box(heads.transpose(2, 3).unwrap().contiguous().unwrap());
    });
    println!("per layer:");
    println!(
        "  one projection  (1500,384)@(384,384)   {:6.2} ms  (x4 = {:6.2})",
        proj * 1e3,
        proj * 4e3
    );
    println!(
        "  split_heads (view only)                {:6.2} ms",
        sh * 1e3
    );
    println!(
        "  .contiguous() on (1,6,1500,64)         {:6.2} ms",
        cont * 1e3
    );
    println!(
        "  transpose(2,3) + .contiguous()  [K]    {:6.2} ms  <- strided gather",
        kt * 1e3
    );
    let glue = proj * 4.0 + cont * 2.0 + kt;
    println!(
        "\n  projections+copies/layer {:6.2} ms  x4 layers = {:6.2} ms",
        glue * 1e3,
        glue * 4e3
    );
    ...
    let scale = t(9, || {
        std::hint::black_box((&heads * 0.125f64).unwrap());
    });
...
RESIDUE CANDIDATES (per layer):"
    ...
    println!(
        "  `* scale` on (1,6,1500,64)   {:6.2} ms  x2 per layer (q and k)",
        scale * 1e3
    );
    println!("  merge: transpose+flatten+cont {:6.2} ms", merge * 1e3);
    let extra = scale * 2.0 + merge;
    println!(
        "  => {:6.2} ms/layer = {:6.2} ms over 4 layers",
        extra * 1e3,
        extra * 4e3
    );
    Ok(())
