    ...
    for tt in 0..HD {
        ...
        for i in 0..rows {
            ...
            while j + 8 <= cols {
                _mm256_storeu_ps(
                    sp.add(j),
                    _mm256_fmadd_ps(qv, _mm256_loadu_ps(kr.add(j)), _mm256_loadu_ps(sp.add(j))),
                );
                j += 8;
            }
        }
    ...
    while i + 4 <= rows {
        ...
        while j + 4 <= cols {
            ...
            while d < HD {
                ...
                for (jj, kvv) in
                    (0..4).map(|jj| (jj, _mm256_loadu_ps(k.as_ptr().add((j + jj) * HD + d))))
                {
                    for ii in 0..4 {
                        acc[ii * 4 + jj] = _mm256_fmadd_ps(qv[ii], kvv, acc[ii * 4 + jj]);
                    }
                }
                d += 8;
            }
...
fn main() {
    ...
    let a = t(2000, || unsafe {
        axpy_form(&q, &kt, &mut s, rows, cols, seq)
    });
    ...
    println!(
        "  AXPY form (needs K transposed) {:7.1} us  {:5.0} GFLOP/s",
        a * 1e6,
        f / a / 1e9
    );
    println!(
        "  NT form   (K natural, no xpose){:7.1} us  {:5.0} GFLOP/s",
        b * 1e6,
        f / b / 1e9
    );
