...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let t2 = t(5, || {
        std::hint::black_box(c2.forward(&h).unwrap());
    });
    let f1 = 2.0 * 80.0 * 384.0 * 3.0 * 3000.0;
    let f2 = 2.0 * 384.0 * 384.0 * 3.0 * 1500.0;
    println!(
        "conv1 (80->384,k3,s1)  {:6.2} ms  {:5.0} GFLOP/s",
        t1 * 1e3,
        f1 / t1 / 1e9
    );
    println!(
        "conv2 (384->384,k3,s2) {:6.2} ms  {:5.0} GFLOP/s",
        t2 * 1e3,
        f2 / t2 / 1e9
    );
    ...
    let g2 = t(5, || {
        std::hint::black_box(a2.matmul(&b2m).unwrap());
    });
    println!("\nsame FLOPs as GEMM (what im2col+matmul would cost, minus the gather):");
    println!(
        "  (3000,240)@(240,384)   {:6.2} ms  {:5.0} GFLOP/s",
        g1 * 1e3,
        f1 / g1 / 1e9
    );
    println!(
        "  (1500,1152)@(1152,384) {:6.2} ms  {:5.0} GFLOP/s",
        g2 * 1e3,
        f2 / g2 / 1e9
    );
