...
fn spread(label: &str, mut f: impl FnMut() -> f64) {
    ...
    println!("{label:<26} min {min:7.2} med {med:7.2} max {max:7.2} ms   spread {:5.0}%",
        (max - min) / med * 100.0);
}
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    spread("encoder fc1 (compute)", || {
        let t = Instant::now(); let o = a.matmul(&b).unwrap(); std::hint::black_box(&o);
        t.elapsed().as_secs_f64() * 1e3
    });
    spread("1024^3 matmul (compute)", || {
        let t = Instant::now(); let o = big.matmul(&big).unwrap(); std::hint::black_box(&o);
        t.elapsed().as_secs_f64() * 1e3
    });
    spread("16M copy (memory)", || {
        let t = Instant::now(); let o = big.copy().unwrap(); std::hint::black_box(&o);
        t.elapsed().as_secs_f64() * 1e3
    });
