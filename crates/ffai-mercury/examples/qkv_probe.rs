...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ktr = t(7, || {
        std::hint::black_box(k4.transpose(2, 3).unwrap().contiguous().unwrap());
    });

    // B: fused, SAME orientation -- (1500,384)@(384,1152), split is strided
    let b = t(7, || {
        std::hint::black_box(x.matmul(&w3).unwrap());
    });
    let fused = x.matmul(&w3)?;
    let split = t(7, || {
        for i in 0..3 {
            std::hint::black_box(fused.narrow(1, i * n, n).unwrap().contiguous().unwrap());
        }
    });
    ...
    let xt_cost = t(7, || {
        std::hint::black_box(x.t().unwrap().contiguous().unwrap());
    });
    let c = t(7, || {
        std::hint::black_box(w3t.matmul(&xt).unwrap());
    });

    println!("A  3 separate matmuls      {:6.2} ms", a * 1e3);
    println!(
        "   + K transpose           {:6.2} ms   => {:6.2} ms",
        ktr * 1e3,
        (a + ktr) * 1e3
    );
    println!("B  fused (1500,384)@(384,1152) {:6.2} ms", b * 1e3);
    println!(
        "   + 3 strided splits      {:6.2} ms   => {:6.2} ms",
        split * 1e3,
        (b + split) * 1e3
    );
    println!("C  x^T once                {:6.2} ms", xt_cost * 1e3);
    println!(
        "   fused (1152,384)@(384,1500) {:6.2} ms  <- splits CONTIGUOUS, K free",
        c * 1e3
    );
    println!(
        "                                       => {:6.2} ms",
