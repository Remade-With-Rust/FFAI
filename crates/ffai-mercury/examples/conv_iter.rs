...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let c1 = Conv1d::new(w.clone(), None, Conv1dConfig { padding: 1, ..Default::default() });
    let base = t(7, || { std::hint::black_box(c1.forward(&mel).unwrap()); });
    ...
    let gemm = t(7, || { std::hint::black_box(wm.matmul(&colt).unwrap()); });
    let f = 2.0 * cout as f64 * (cin * 3) as f64 * l as f64;
    println!("candle conv1d          {:6.2} ms   {:5.0} GFLOP/s", base*1e3, f/base/1e9);
    println!("im2col build (contig)  {:6.2} ms", build*1e3);
    println!("GEMM (384,240)@(240,3000) {:6.2} ms {:5.0} GFLOP/s", gemm*1e3, f/gemm/1e9);
    println!("  im2col + GEMM total  {:6.2} ms  -> {:.2}x vs candle conv1d", (build+gemm)*1e3, base/(build+gemm));
    ...
    let cv2 = Conv1d::new(w2.clone(), None, Conv1dConfig { padding: 1, stride: 2, ..Default::default() });
    let b2 = t(7, || { std::hint::black_box(cv2.forward(&h).unwrap()); });
    ...
    let g2 = t(7, || { std::hint::black_box(wm2.matmul(&col2).unwrap()); });
    let f2 = 2.0 * c2i as f64 * (c2i * 3) as f64 * lo as f64;
    println!("
conv2 (stride 2):");
    println!("  candle conv1d        {:6.2} ms   {:5.0} GFLOP/s", b2*1e3, f2/b2/1e9);
    println!("  im2col build (strided reads) {:6.2} ms", build2*1e3);
    println!("  GEMM (384,1152)@(1152,1500)  {:6.2} ms  {:5.0} GFLOP/s", g2*1e3, f2/g2/1e9);
    println!("  total {:6.2} ms -> {:.2}x vs candle", (build2+g2)*1e3, b2/(build2+g2));
    Ok(())
}
