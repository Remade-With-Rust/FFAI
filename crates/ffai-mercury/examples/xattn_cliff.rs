...
fn best(n: usize, mut f: impl FnMut()) -> f64 {
    f(); let mut b=f64::MAX;
    for _ in 0..n { let t=Instant::now(); f(); b=b.min(t.elapsed().as_secs_f64()); } b
}
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for m in [1usize,2,4,8] {
        ...
        let tqk = best(200, || { std::hint::black_box(q.matmul(&k).unwrap()); });
        let twv = best(200, || { std::hint::black_box(s.matmul(&v).unwrap()); });
        println!("  m={m}: q@k {:7.2} us   w@v {:7.2} us   sum {:7.2} us", tqk*1e6, twv*1e6, (tqk+twv)*1e6);
    }
    Ok(())
}
