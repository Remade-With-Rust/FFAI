...
| H-10 | ★ `cargo vet` coverage complete | Incomplete | `cargo vet` initialised and **enforced per-PR** in the `supply-chain` CI job; `cargo vet check` passes. Seven third-party audit sources imported (mozilla, google, bytecode-alliance, embark-studios, zcash, isrg, fermyon), covering **98 fully audited + 3 partially audited of 494 third-party crates**. The other **474 are EXEMPTIONS, not certifications** — a written acknowledgement that a crate is trusted only because it was already in the tree. The criterion asks for every dependency to be covered by an audit, so this stays open; marking it met would be the "checklist that records intentions" the standard warns against. **What it does buy today is the ratchet**: a NEW dependency now fails CI unless an imported audit covers it or a reviewed exemption is added, so the gap cannot grow while it is burned down. Burn-down procedure and prioritisation in `supply-chain/README.md` | |
...
| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-15 | ★ Workspace lint policy set and clean | Incomplete | **five crates now clippy-clean and BLOCKING in CI** — ffai-core, ffai-models, ffai-media, ffai-argus, ffai-mercury — up from one. Measuring first paid: core, models and argus were already at zero and cost nothing to enforce, and ffai-media's "4 findings" were really 3 in one file (ffai-bench merely inherits them). Fixed there: a manual `%`-vs-`is_multiple_of`, a `let vidx = vidx;` self-rebind, and `from_rff_frame` marked dead-code-with-reason rather than deleted, since it holds the BT.601 colour contract the video path will need. Remaining: ffai-bench 8, ffai-diana 30, ffai-carmenta 34, ffai-cli 64 (binary-only, so `--lib` does not apply). Gate still asks for pedantic + nursery, which no crate has yet attempted | |
...
| H-27 | ★ Continuous fuzzing with no open crashes | Incomplete | no long-running campaign yet, but the targets are no longer untested: structure-aware **protobuf fuzzing runs in CI** via `tests/properties.rs` (a generator emitting well-framed messages with arbitrary field numbers, wire types, nesting and truncations — random bytes never get past the framing), plus targeted hostile-dims cases. That found the `product()` overflow. The corpus carries 6 regression seeds. Gate needs 30 days of coverage-guided fuzzing, which needs cargo-fuzz on a Linux runner | |
