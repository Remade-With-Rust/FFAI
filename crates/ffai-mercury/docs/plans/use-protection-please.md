...
| H-12 | ★ SBOM generated and published with releases | Completed | `release.yml` generates a CycloneDX 1.5 SBOM per crate and attaches it to the release; `cargo auditable` additionally embeds the dependency list inside each binary, so a shipped artifact can be scanned directly with `cargo audit bin`. Default features only — `--all-features` pulls candle's `metal` and the macOS-only `objc2`, which cannot resolve on a Linux runner. **Blocked on the same fact as H-38**: no release exists yet to attach an SBOM to **MET 2026-08-19 by the v0.7.0 release.** `release.yml` ran on the signed tag and published **13 per-crate CycloneDX SBOMs** (`ffai-*.cdx.json`) as release assets, alongside three `cargo auditable` binaries that carry their own dependency manifest INSIDE the executable — so `cargo audit bin` can scan a shipped file even when it is separated from the release page. 14 assets total | |
...
| H-38 | Releases signed, attested, and changelogged for security | Completed | `.github/workflows/release.yml` implements the whole path — a **signed-tag gate** that fails the release if `git cat-file tag` shows no PGP/SSH signature, a `cargo auditable` build so binaries carry their own dependency manifest, SLSA **build provenance** via `actions/attest-build-provenance` (OIDC-signed), and `checksec` on the Linux binary. `CHANGELOG.md` defines the Security-subsection convention this gate asks for, including the rule that "no security-relevant changes" must be stated rather than implied. **Blocked, not unbuilt**: the repository has no tags and no releases, so nothing has been signed or attested yet. Flips on the first verified release **MET 2026-08-19.** v0.7.0 is an **annotated, SSH-signed tag**, and `release.yml`'s `verify-tag` job now genuinely verifies it. Getting there found the gate itself was broken: it reported a correctly signed tag as unsigned, because `actions/checkout` leaves a detached HEAD and `git cat-file tag <name>` resolved nothing — GitHub's own API confirmed the tag was annotated with a signature present. **A gate that rejects valid input is as corrosive as one that accepts invalid input**, so it now fetches the tag object explicitly and distinguishes "lightweight tag" from "unsigned". SLSA provenance is attested over all four artifacts via `actions/attest-build-provenance` and OIDC. CHANGELOG carries the Security subsection. **Caveat, deliberate**: the SSH key is not registered on the GitHub account, so the signature verifies but shows as `unknown_key` rather than *Verified*. Registering it back-fills the badge on this existing tag with nothing to redo | |
...
| C-12 | Change management — reviewed, approved, traceable | Completed | CI now runs on every PR, but branch protection and a required-review rule are **not** configured on the repo, so traceability is convention rather than enforcement — a settings change, not a code change **MET 2026-08-19.** Branch protection is enabled on `master`: **all 8 CI checks required and strict**, pull requests mandatory, force-pushes and branch deletion blocked, conversation resolution required. Every change since is traceable through a PR with its checks recorded — this audit's own work landed as PR #1, #12 and #13. Two settings chosen deliberately and worth stating: `enforce_admins: false` and `required_approving_review_count: 0`, so a solo maintainer is not locked out of their own repository while every change still passes through a PR with all 8 checks green. Tighten both the moment there is a second reviewer | |
| C-13 | Availability commitments and their evidence | N/A | library; no SLA or availability commitment is made by this crate | |
| C-14 | Machine-readable SBOM + provenance for regulators | Completed | same machinery as H-12: CycloneDX 1.5 JSON per crate, plus an OIDC-signed SLSA provenance attestation verifiable with `gh attestation verify`. That is the format and the chain of custody a regulator asks for (EU CRA Art 13). **Blocked**: needs a release to exist **MET 2026-08-19.** Machine-readable CycloneDX SBOMs (13, one per crate) plus SLSA build provenance attested over every published artifact — digests recorded in the run: three `ffai-*` binaries and the SBOM bundle. Both are the formats a regulator or downstream consumer can actually ingest, produced by the release pipeline rather than by hand | |
...
| R-006 | **`ffai-carmenta` and `ffai-diana` allow the three narrowing-cast lints wholesale** — 149 and 38 sites, none individually reviewed — while every other crate cleared them one at a time against the trust boundary. Both crates parse images and documents, i.e. more untrusted bytes than mercury | **Medium-High** (the same review found defects everywhere it has been run) | Medium (silent wrong answer, not a panic — `overflow-checks` is off per R-005) | **RE-SCOPED 2026-08-19: OUT OF SCOPE FOR THIS UNIT.** This audit's unit is `ffai-mercury`; carmenta and diana are separate crates and the project lead has scheduled a **repo-wide FFai audit** to cover them rather than folding them into mercury's. Recorded here only because mercury's H-15 work is what measured the exposure, and because it is R-003's bug class on a wider surface. Carried forward, NOT closed: both crates' `Cargo.toml` state the position at the allow block, and `supply-chain/WORKLIST.md` plus this row hold the numbers for whoever runs that pass | Tim — deferred to the FFai-wide audit | (that audit's date) |
