...
| H-10 | ★ `cargo vet` coverage complete | Incomplete | **exemptions 473 -> 331, fully audited 98 -> 242** via `cargo vet trust`, the mechanism `cargo vet suggest` recommends and I had overlooked. 128 trusted entries across 11 publishers — dtolnay (23 crates), Manishearth (21), kennykerr (21), epage (18), seanmonstar (14), BurntSushi (12), cuviper (8), Amanieu (5), sunfishcode (4), sfackler, alexcrichton — each trusted **because other imported organisations already trust them** (`isrg, mozilla, ariel-os and 1 other trust ...`), and each recorded with the publisher's user-id and a validity window. Iterated to convergence: round 3 produced no new candidates. Context for the remaining 331: `cargo vet suggest` puts the hand-audit backlog at **10,964,491 lines**, so trust and dependency removal are the only realistic levers — 135 of the remainder are the optional downloader (see `supply-chain/README.md`) | |
...
| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-15 | ★ Workspace lint policy set and clean | Incomplete | **six crates clippy-clean and BLOCKING**: ffai-core, ffai-models, ffai-media, ffai-argus, ffai-mercury, ffai-bench — up from one. ffai-bench's last four were fixed by hand: a never-used `duration` kept with a reason (every future turn-weighted metric wants it), and two `child.as_raw_handle() as *mut c_void` casts that were redundant because `as_raw_handle` already returns that type. Remaining: ffai-diana 30, ffai-carmenta 34, ffai-cli 64 (binary-only, so `--lib` does not apply). fmt blocks for mercury + media. Gate still asks for pedantic + nursery, which no crate has attempted | |
...
| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-30 | Proof of panic-freedom / UB-freedom per `unsafe` module | Incomplete | **five bounded Kani harnesses, now covering the unsafe modules and not just helpers.** The two new ones prove R-002 — the last claim in the crate that was argued rather than tested. `unsafe impl Send`/`Sync` for `SendPtr` is sound only if distinct rayon tasks write disjoint regions; that reasoning lived in a comment. The index arithmetic is now extracted into `decoder_kernels::task_region`, which the kernel actually calls, and two harnesses prove over it that (a) distinct tasks are separated on at least one axis, and (b) every region is in bounds with `t1 - t0` never underflowing. Kani cannot execute the AVX2 kernel and does not need to: soundness depends on WHERE each task may write, not on what the intrinsics compute. Still **advisory** in CI — Kani does not run on Windows, so none of the five has been executed | |
...
| R-004 | CI exists (H-37) but `fmt` and `clippy` block only for `ffai-mercury`, and branch protection is not configured (C-12), so a regression in another crate can still land | Medium | Medium | **ACCEPTED 2026-08-15** — test, supply-chain, Miri, invariants and table-freshness block for everyone; per-crate adoption widens as crates are cleaned. Branch protection deliberately deferred | Tim | 2026-09-30 |
