...
| H-19 | Input validation — external bytes treated as hostile | Incomplete | the untrusted surface is now caller audio and text (model files are trusted — R-001). `transcribe` validates `sample_rate` and rejects mismatches. The audio front end is property-tested as total (H-28) and one real out-of-bounds panic on empty input was found and fixed. **Still unaudited**: `channels == 0` and other `AudioBuffer` field combinations end to end, which needs cached weights to exercise | |
...
| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-23 | ★ Tests pass under Miri | Incomplete | **RUN, and the blocker is now measured rather than assumed.** `cargo +nightly miri test -p ffai-mercury --lib` aborts on `asr::adaptive::tests::an_explicit_override_wins`. The cause is **Win32 FFI, not SIMD**: the memory-instrumentation path declares `unsafe extern "system"` and calls `SetProcessWorkingSetSizeEx`, `GetCurrentProcess`, `K32GetProcessMemoryInfo` and `CreateJobObjectW`, and Miri cannot execute foreign functions. (An earlier note in this file guessed the AVX2 intrinsics would be the blocker — they are reached later, if at all.) Concrete path: run Miri on a **Linux** CI runner, where that Win32 path is not compiled at all | |
