...
| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-15 | ★ Workspace lint policy set and clean | Completed | **TEN of twelve crates are clean under clippy pedantic + nursery**, each enforced by its own manifest and blocking in CI: ffai-core, ffai-models, ffai-media, ffai-argus, ffai-mercury, ffai-bench, ffai-cli, ffai-demo, ffai-py, ffai-wasm. Totals cleared: 94 + 96 + 108 + 159 + 253 + 757 + 38 + 22 + 10 + 5. The remaining two — ffai-carmenta and ffai-diana — are **deliberately deferred at the user's instruction**, not unaddressed. The casts were split by TRUST BOUNDARY rather than blanket-allowed: truncation/sign-loss/wrap stay DENIED on the untrusted surface and are allowed only where a guard is named in writing (`.clamp()` before the cast, `is_finite()`, a bounded `match` arm, a const table index). **That policy found four defects** — ONNX field-number aliasing, the WAV `bits_per_sample` shift, three unchecked frame allocations, and a zero-sample-rate divide that saturated into `Vec::with_capacity(usize::MAX)` — plus a dropped job-assignment result that silently invalidated footprint measurements. CI uses `--no-deps`, without which `-D warnings` promotes the two deferred crates' warnings and reds the gate on code it is not checking | |
