...
| R-002 | `SendPtr` disjointness is argued in a comment and at the site, not proven. A wrong band calculation is a data race in released code, not a panic | Low | High (UB, silently wrong audio) | **ACCEPTED 2026-08-15, evidence added 2026-08-18** — the argument is written at the site and in `UNSAFE.md` class B; Miri cannot reach it (SIMD). It now has its first EXECUTED check: `proofs::exhaustive::flash_head_bands_are_disjoint_and_in_bounds` enumerates every (head, row, col) for hd/heads/seq up to 3 and asserts no two heads produce the same write index. That is exhaustive for the bounded domain, not general — the Kani harness covering the same claim still needs a Linux runner, and TSan (H-24) remains the outstanding proof at full width | Tim | 2026-11-15 |
