...
| H-10 (partial) | **`aws-lc-sys` only** — not the gate as a whole. One crate is 39% of the audit backlog, is `fetch`-only so absent from any default build, is AWS-LC with its own external audit and FIPS validation, and cannot be removed from this repo because `hf-hub` forces the provider upstream. Reviewing 2.1M lines of vendored C and assembly by hand would consume the entire remaining budget for the weakest return in the list. The other 312 exemptions are NOT waived and stay on the worklist | Tim (project lead) | 2026-11-15, or immediately on any RUSTSEC advisory against it, or when the upstream `hf-hub` feature lands |
