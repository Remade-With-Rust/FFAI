...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Name the corpus after the split the archive actually contained. This
    // was hardcoded to test-clean, so the first test-other manifest generated
    // was internally labelled test-clean — a corpus that lies about its own
    // identity is worse than no second corpus at all.
    let split_name = root
        .file_name()
        .and_then(|s| s.to_str())
        .unwrap_or("unknown");
    let manifest = format!(
        ...
        "# LibriSpeech {split_name} subset - generated by\n\
         #   cargo run -p ffai-bench --example prepare_librispeech\n\
         # Source: https://www.openslr.org/resources/12/{split_name}.tar.gz\n\
